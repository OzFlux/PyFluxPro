...
sites = [s for s in processing_sheet.col_values(0)[1:]]
processing_info = {}
for row, site in enumerate(sites):
    processing_info[site] = {}
    for col, label in enumerate(labels[1:]):
        processing_info[site][label] = processing_sheet.cell_value(row+1, col+1)
...
for site in sites:
    ...
    bp_folder = processing_info[site]["BP folder"]
    bp_name =  processing_info[site]["BP name"]
    uqrdm_folder = processing_info[site]["UQRDM folder"]
    uqrdm_name = processing_info[site]["UQRDM name"]
    uqrdm_folders = [uqrdm_folder, "Data", "Flux", "Processed", uqrdm_version]
    for item in uqrdm_folders:
        uqrdm_path = os.path.join(uqrdm_path, item)
    ...
    for level in levels:
        ...
        if not check_uqrdm_folder(uqrdm_method_path, create=create, dry_run=dry_run):
            continue
        source_path = os.path.join(bp_base, bp_folder, "Data", "Processed")
        os.chdir(source_path)
        source_name = bp_name + "_" + level + ".nc"
        destination_name, ok = get_destination_name(source_name)
        if not ok:
            continue
        uqrdm_path = os.path.join(uqrdm_base, uqrdm_folder, "Data", "Flux", "Processed")
