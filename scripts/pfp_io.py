...
def ReadCSVFile(l1_info):
    ...
    for n, file_name in enumerate(file_names):
        ...
        # are we dealing with a FluxNet HH file?
        if ("TIMESTAMP_END" in headers):
            ...
            df["TIMESTAMP"] = pandas.to_datetime(df["TIMESTAMP_END"].astype("string"),
                                                 errors="raise")
        # or maybe a FluxNet DD file?
        elif ("TIMESTAMP" in headers):
            # if so, we use the TIMESTAMP column
...
def write_tsv_reddyproc(cfg):
    ...
    for label in list(cfg["Variables"].keys()):
        data[label] = {"name": cfg["Variables"][label]["name"],
                       "format": cfg["Variables"][label]["format"]}
