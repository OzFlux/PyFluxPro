...
def read_isd_file_csv(isd_file_path):
    ...
    if time_step not in [10, 30, 60, 180]:
        ...
        ds_its.globalattributes["time_step"] = int(scipy.stats.mode(dt/float(60))[0][0])
    ...
    RH["Attr"] = {"long_name": "Relative humidity", "statistics_type": "average",
    ...
    AH["Data"] = mf.absolutehumidityfromrelativehumidity(Ta["Data"], RH["Data"])
    AH["Flag"] = numpy.where(numpy.ma.getmaskarray(AH["Data"]) == True, ones, zeros)
    AH["Attr"] = {"long_name": "Absolute humidity", "statistic_type": "average",
    ...
    SH["Data"] = mf.specifichumidityfromrelativehumidity(RH["Data"], Ta["Data"], ps["Data"])
    SH["Flag"] = numpy.where(numpy.ma.getmaskarray(SH["Data"]) == True, ones, zeros)
    SH["Attr"] = {"long_name":"Specific humidity", "statistic_type": "average",
...
for site in site_list:
    ...
    for n, year in enumerate(year_list):
        ...
        for isd_site in isd_site_list:
            ...
            isd_file_path = os.path.join(isd_year_path, isd_site.replace("-", "") + ".csv")
            if not os.path.isfile(isd_file_path):
                msg = " File not found: " + isd_file_path
                logger.error(msg)
                continue
